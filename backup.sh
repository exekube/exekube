...
source=("ci.x${domain_zone}-tls" "wp.${domain_zone}-tls" "react.${domain_zone}-tls" "registry.${domain_zone}-tls" "charts.${domain_zone}-tls")
green="\e[1;32m"
no_color="\e[0m"
...
for i in ${source[@]}
do
  echo "---" >> /exekube/backup/tls/secret-$(date '+%Y-%m-%d-%H-%M-%S').yaml \
